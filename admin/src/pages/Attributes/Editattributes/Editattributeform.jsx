...
import { useGetSingleAttributeQuery, usePatchAttributeMutation, usePostAttributeMutation } from "../../../store/api/attributeapi";
const Editattributeform = ({id}) => {

  const { data, isLoading } = useGetSingleAttributeQuery(id);
  const nvg = useNavigate();
  const [patchAttribute] = usePatchAttributeMutation();
  const AttributeForm = async (value, setFieldError) => {
    try {
      const response = await patchAttribute({ data: value, id: id });
      if (!response.error) {
    ...
    <div className="container-fuild pb-4 pt-3 px-2 bg-white">
      <Formik
        initialValues={{
          ...
          attributeValues: data.data.attributeValues,
        }}
